...
export class LibraryService {
    ...
    removeUser(user: User): void {
        ...
        books.forEach((book) => {
            book.available = user.borrowedBooks.find((b) => b.id === book.id)
                ? true
                : book.available;
        });
        ...
        this.storage.saveUsers(
            this.storage.getUsers().filter((u) => u.id !== user.id),
        );
    }
    ...
    updateUser(oldUser: User, newUser: User): void {
        this.storage.saveUsers([
            newUser,
            ...this.storage.getUsers().filter((u) => u.id !== oldUser.id),
        ]);
    }
    ...
    renderBookList(books?: Book[], itemsPerPage?: number): string {
        if (books.length > 0)
                ...
                <ul class="list-group">
                    ${books
                        .map(
                            (
                                book,
                            ) => `<li title="Delete book" class="list-group-item bookList d-flex justify-content-between align-items-center" bookid="${book.id}">
                        ${book.title} by ${book.author} in year ${book.year}
            ...
            `;
        else return this.renderPaginatedBookList(1, itemsPerPage);
    }
                        ...
                        <ul class="list-group inner-list">
                            ${
                                user.borrowedBooks.length === 0
                                    ? `<li class="list-group-item">No borrowed books</li>`
                                    : user.borrowedBooks
                                          .map(
                                              (book) => `
                                    <li class="list-group-item userBook" bookid="${book.id}">Book: ${book.title} by ${book.author}</li>
                                `,
                                          )
                                          .join("")
                            }
    ...
    private renderPagination(
        ...
        let paginationHTML =
            '<nav aria-label="Book list pagination"><ul class="pagination">';
